...
def freeze_layers(model, num_layers=0):
    ...
    #     # Check if the parameter belongs to a convolutional layer
    #     if "conv" in name:
    #         if count < num_layers:
    #             param.requires_grad = False
    #             count += 1
    #         else:
    #             break  # Stop freezing once the specified number of layers is reached
    if num_layers == 0:
        return
        
    for name, param in model.named_parameters():
        if 'conv1' in name and num_layers >= 1:
            param.requires_grad = False
        elif 'conv2' in name and num_layers >= 2:
            param.requires_grad = False
        elif 'conv3' in name and num_layers >= 3:
            param.requires_grad = False
