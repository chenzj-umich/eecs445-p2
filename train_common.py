...
def early_stopping(stats, curr_count_to_patience, prev_val_loss):
    ...
    # Extract the latest validation loss from stats
    current_val_loss = stats[-1][1]  # Assuming validation loss is at index 1 in the stats list

    if current_val_loss < prev_val_loss:
        # Validation loss decreased, reset patience counter and update previous loss
        curr_count_to_patience = 0
        prev_val_loss = current_val_loss
    else:
        # Validation loss did not improve, increment patience counter
