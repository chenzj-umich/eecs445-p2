...
class ImageStandardizer(object):
    ...
    def fit(self, X):
        ...
        """Calculate per-channel mean and standard deviation from dataset X.
        Args:
            X: A numpy array of shape (N, image_height, image_width, 3).
        """
        # Compute the mean and standard deviation across the batch (axis 0, 1, 2 for N, H, W)
    ...
    def transform(self, X):
        ...
        Returns:
            Standardized dataset with zero-centered and scaled values.
        """
        if self.image_mean is None or self.image_std is None:
            raise ValueError("Must fit the standardizer on training data before calling transform.")
        
        # Zero-center by subtracting the mean and scaling by the standard deviation
